...
import Biru from './number1'
import Ijo from './number2'
import Oren from './number3'
import Merah from './number4'
import Link from 'next/link';
import { useState } from "react";

export default function ndatau() {
        ...
        return (
          ...
          <nav className="block w-full max-w-screen px-4 py-4 mx-auto bg-white bg-opacity-90 sticky top-3 shadow lg:px-8 backdrop-blur-lg backdrop-saturate-150 z-[9999]">
            ...
            <div className="container flex flex-wrap items-center justify-between mx-auto text-slate-800">
              ...
              <div className="lg:hidden">
                <button
                  className="relative ml-auto h-6 max-h-[40px] w-6 max-w-[40px] select-none rounded-lg text-center align-middle text-xs font-medium uppercase text-slate-700 transition-all disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none"
            ...
            <div className="hidden lg:block">
              <ul className="flex flex-col gap-2 mt-2 mb-4 lg:mt-0 lg:mb-0 lg:flex-row lg:items-center lg:gap-6">
                {navItems.map((item, index) => (
                  <li
                    key={index}
                    className="flex items-center p-1 text-lg gap-x-2 text-slate-600 hover:text-red-500"
          ...
          <div className='flex flex-row p-11 bg-gray-100'>
          <Biru/>
          <Ijo/>
          <Oren/>
          <Merah/>
